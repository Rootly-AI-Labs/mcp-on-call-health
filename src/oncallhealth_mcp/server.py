...
from oncallhealth_mcp.normalizers import (
    ...
    normalize_member_daily_health,
    normalize_oncall_users,
...
async def validate_integrations(
    ...
    Call this before starting an analysis to verify all tokens are active.
    Returns per-integration validation status (GitHub, Slack, Jira, Linear).

    Args:
        force_refresh: If True, bypass cache and make fresh API calls (default: False)

    Returns:
        - all_valid: True if every integration has valid credentials
        - integrations: dict mapping integration name to {valid, message, error_code?}

    Example:
        >>> result = await validate_integrations(ctx)
        >>> if not result['all_valid']:
        ...     print("Fix integrations before running analysis")
    """
    api_key = _validate_api_key(ctx)
    ...
    async with OnCallHealthClient(api_key=api_key) as client:
        ...
        response = await client.get("/analyses/validate-integrations", params=params)
        data = response.json()
        return normalize_validation_response(data)
...
async def oncall_users(
    ...
    Use this to check who is currently on-call, then cross-reference
    with analysis data to see if any on-call responders are at risk.

    Args:
        integration_id: Rootly integration ID (from integrations_list)

    Returns:
        - integration_id: the integration queried
        - total_oncall: count of on-call users
        - oncall_emails: list of email addresses currently on-call
        - checked_at: ISO timestamp of the check

    Example:
        >>> result = await oncall_users(ctx, integration_id=1)
        >>> print(f"{result['total_oncall']} people currently on-call")
    """
    api_key = _validate_api_key(ctx)
    if integration_id <= 0:
        raise ValueError(f"integration_id must be positive, got {integration_id}")
    ...
    async with OnCallHealthClient(api_key=api_key) as client:
        try:
            response = await client.get(
                f"/rootly/integrations/{integration_id}/oncall-users"
            )
            data = response.json()
            return normalize_oncall_users(data)
        except NotFoundError:
            raise LookupError("Integration not found")
...
async def member_daily_health(
    ...
    Use this to investigate a specific person's health trends over the
    analysis period. Shows per-day health scores, incident counts,
    and after-hours activity.

    Args:
        analysis_id: The analysis to query
        member_email: Email address of the team member

    Returns:
        - member_email: the member queried
        - member_name: display name
        - daily_health: list of {date, health_score, incident_count,
                        after_hours_count, severity_weighted_count}
                        (only days with data included)
        - summary: {total_days, days_with_data, avg_health_score}

    Example:
        >>> result = await member_daily_health(ctx, 1226, "alice@example.com")
        >>> print(f"Average health: {result['summary']['avg_health_score']}")
    """
    api_key = _validate_api_key(ctx)
    _validate_analysis_id(analysis_id)
    if not member_email or not member_email.strip():
        raise ValueError("member_email cannot be empty")

    async with OnCallHealthClient(api_key=api_key) as client:
        try:
            response = await client.get(
                f"/analyses/{analysis_id}/members/{member_email}/daily-health"
            )
            data = response.json()
            if data.get("status") == "error":
                raise LookupError(data.get("message", "Member not found"))
            return normalize_member_daily_health(data)
        except NotFoundError:
            raise LookupError("Analysis or member not found")
